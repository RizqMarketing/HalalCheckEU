...
class EventBus {
     ...
     */
    subscribe(eventType, handler, source) {
        const subscription = {
     ...
     */
    async emit(eventType, data, source, target) {
        const event = {
     ...
     */
    waitForEvent(eventType, timeout = 30000, filter) {
     ...
     */
    async request(requestType, responseType, data, timeout = 30000) {
        const requestId = `req_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        // Set up response listener first
        const responsePromise = this.waitForEvent(responseType, timeout, (event) => event.data.requestId === requestId);
        // Send the request
